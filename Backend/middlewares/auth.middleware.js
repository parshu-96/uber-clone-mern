...
module.exports.authCaptain = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET); // Verify the token
    req.captain = await captainModel.findById(decoded._id); // Find the captain by ID from the token
    if (!req.captain) {
      return res.status(401).json({ error: "Unauthorized" }); // Return an error if the captain is not found
    }
    next(); // Call the next middleware or route handler
  } catch (error) {
    return res.status(401).json({ error: "Unauthorized" }); // Return an error if the token is invalid
